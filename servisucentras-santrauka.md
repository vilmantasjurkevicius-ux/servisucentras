...
- Naujas CSS `.hero-flag-car` — pradžioje centruota (`top:50%;left:50%;transform:translate(-50%,-50%)`), vėliau vartotojo prašymu perkelta į KAIRĘ vėliavos dėžutės pusę.
- Pridėta `.hero-flag-car-box` — puslapio fono spalvos (`var(--bg)`) dėžutė aplink automobilį, kad nesimatytų krašto skirtumo.
- **Galutinis sprendimas — hero pertvarkytas į 2 stulpelius**: vartotojas paaiškino, kad automobilis turi būti ŠALIA vėliavos, NE ant jos (ankstesnis variantas dengė kairę vėliavos pusę, nes abu dalinosi ta pačia absoliučiai pozicionuota fono zona). Patikrinus tiksliai (`getBoundingClientRect()` + Range API antraštės tekstui) — prie 1280px pločio tarp teksto (baigiasi x≈846) ir vėliavos (prasideda x≈865) buvo tik ~19px, nepakankamai automobiliui.
  - `.hero-inner` tapo CSS grid (`grid-template-columns:1fr auto`), teksto turinys suvyniotas į naują `.hero-text`, o vėliava+automobilis — į naują `.hero-visual` (flex, gap 16px): automobilio dėžutė (150×190) KAIRĖJE, pilna vėliava (sumažinta nuo 440×294 iki 300×200, tas pats kraštinių santykis) DEŠINĖJE — abu greta, jokio persidengimo.
  - `@media(max-width:960px)` — `.hero-visual` paslėptas, `.hero-inner` grąžinamas į 1 stulpelį (mobiliuose vėliava+auto ir taip būtų per ankšta).
  - Šalutinis efektas: antraštė ("Sugedo jūsų automobilis?...") dabar siauresniame stulpelyje (602px vietoj ~1100px) ir lūžta į 4 eilutes vietoj 3 — patikrinta per Range API, kad žodžiai lūžta tvarkingai tarp žodžių (pvz. "Sugedo jūsų" / "automobilis?"), ne per vidurį žodžio.
  - Patikrinta tiksliais matavimais (Browser pane screenshot vartotojo pusėje nebuvo rodomas, todėl remtasi `getBoundingClientRect()`/Range API skaičiavimais + widget peržiūra su tikru PNG): automobilio dėžutė x:716–866, vėliava x:882–1182 (16px tarpas), tekstas baigiasi x≈642 (~74px tarpo iki vaizdo) — jokio persidengimo. Console klaidų nėra.
- **Padidinta vėliava + automobilio fonas suvienodintas su svetaine (2026-07-31)**: `.hero-flag-bg` padidinta iš 300×200 iki 360×240 (tas pats 1.5 kraštinių santykis). `.hero-flag-car-box` fonas pakeistas iš `var(--bg)` (vientisa spalva) į `transparent` — anksčiau ši dėžutė BLOKAVO `.hero::before` subtilų fono efektą (radialinį švytėjimą + įstrižą tekstūrą), todėl aplink automobilį matėsi lygus stačiakampis, kitoks nei likusi hero sekcija. Dabar per skaidrią dėžutę matosi TAS PATS fono efektas kaip visur — jokio matomo krašto. Šalutinis efektas: teksto stulpelis susiaurėjo iki 542px, antraštė lūžta į 5 eilutes vietoj 4 (patikrinta Range API — vis tiek lūžta tarp žodžių, ne per vidurį).
- **Papildomas padidinimas (2026-07-31, tas pats pokalbis)**: vartotojas atsiuntė production ekrano nuotrauką su klausimu, kodėl tarpas tarp auto ir vėliavos atrodo didelis. Patikrinus TIESIOGIAI `servisucentras-production.up.railway.app` per `getBoundingClientRect()` — production jau atitiko kodą tiksliai (16px tarpas), taigi nuotraukos vaizdas buvo tik ekrano nuotraukos mastelio artefaktas, NE reali CSS klaida. Vis tiek papildomai padidinta vėliava 360×240 → 420×280, tarpas tarp auto/vėliavos 16px → 24px (`.hero-visual` gap). Patikrinta 1280px pločiu: tekstas baigiasi x≈471, auto x:588–738, vėliava x:762–1182 — jokio persidengimo, tas pats 5 eilučių antraštės lūžis.
- **Maksimalus padidinimas + auto pritraukimas prie teksto (2026-07-31)**: vartotojas paprašė padidinti vėliavą KIEK TIK ĮMANOMA ir pritraukti automobilį arčiau antraštės teksto. Realią ribą nustato ilgiausias NELŪŽTANTIS antraštės žodis — `"automobilis?"` prie 64px šrifto užima 388.75px (išmatuota tiksliai per Range API `getBoundingClientRect()`) — jei teksto stulpelis susiaurėtų žemiau šios ribos, žodis persidengtų su automobiliu/vėliava arba priverstų grid'ą išsiplėsti.
  - Vėliava padidinta iki 500×333 (nuo 420×280) — didžiausias dydis, paliekantis ~30px saugumo maržą virš 388.75px ribos (teksto stulpelis lieka ~412–422px, priklausomai nuo lango pločio).
  - `.hero-inner` grid gap sumažintas 32px → 12px (automobilis dabar arčiau teksto) ir `.hero-visual` gap 24px → 16px (auto-vėliava tarpas).
  - **Svarbu**: kadangi vizualas užima daugiau vietos, mobiliojo režimo riba PERKELTA `960px → 1160px` — priešingu atveju tarp 960–1160px būtų atsiradusi "pavojaus zona", kur teksto stulpelis susiaurėtų žemiau 388.75px ribos ir žodis "automobilis?" persidengtų su automobiliu/vėliava.
  - Patikrinta tiksliais matavimais KELIUOSE pločiuose: 1280px (teksto stulpelis 422px, žodžio riba x=471.25, stulpelio riba x=504.5 — 33px marža), 1161px (tiesiai virš naujos ribos — teksto stulpelis 412px, 24px marža), 1160px (riba — vizualas pilnai paslėptas, `grid-template-columns` grąžta į 1 stulpelį). Jokio horizontalaus scroll'inimo (`document.documentElement.scrollWidth <= window.innerWidth`) visuose trijuose atvejuose. Console klaidų nėra.
- **Bandymas: `hyphens:auto` antraštei (2026-07-31) — NEVEIKIA lietuviškai, paliktas kaip neškodingas**: vartotojas norėjo dar didesnės vėliavos, pasiūlius leisti žodžiams lūžti su brūkšneliu (vietoj šrifto mažinimo). Ištestuota TIESIOGIAI naršyklėje priverstinai susiaurinus teksto stulpelį — žodis "automobilis?" NELŪŽTA su brūkšneliu, o tiesiog IŠLENDA už rėmų (389px turinys 300px dėžutėje). Chromium/Chrome/Edge neturi lietuviško žodžių skiemenavimo žodyno, tad šis metodas nepatikimas daugumai vartotojų. `hyphens:auto` CSS liko `h1` stiliuje (nekenkia, bet nepasitikima juo saugiam dydžio skaičiavimui).
- **Automobilis sumažintas iki raidžių dydžio + pritrauktas prie teksto (2026-07-31)**: vartotojas paprašė vėliavą palikti kaip buvo (500×333), o automobilį patraukti arčiau teksto ir sulyginti su raidžių dydžiu.
  - `.hero-flag-car` aukštis sumažintas nuo 120px pločio iki `height:70px` (artima vienos H1 eilutės aukščiui — 64px šriftas × 0.96 line-height ≈ 61px).
  - `.hero-flag-car-box` nebeturi fiksuoto 150×190 dydžio — dabar tik apgaubia paveikslėlį (`width/height:auto`), nes didelė dėžutė nebereikalinga tokiam mažam vaizdui.
  - `.hero-inner` grid gap sumažintas 12px → 8px (automobilis dar arčiau teksto).
  - Patikrinta 1280px pločiu: teksto stulpelis išsiplėtė iki 488px (nes automobilio dėžutė užima mažiau vietos), automobilis x:579–667 (8px tarpas nuo teksto), vėliava x:683–1183 (16px tarpas nuo auto, dydis nepakitęs). Jokio persidengimo, console klaidų nėra.
- **Automobilis visiškai pašalintas (2026-07-31)**: po kelių iteracijų vartotojas nusprendė automobilio hero sekcijoje visai atsisakyti. Pašalinta: `<img class="hero-flag-car">` + `.hero-flag-car-box` HTML, nebenaudojamas `.hero-flag-car-box`/`.hero-flag-car` CSS, pats `car-white-transparent.png` failas (niekur daugiau nenaudojamas). Liko tik vėliava (`.hero-flag-bg`, 500×333) `.hero-visual` konteineryje. Patikrinta: elementų DOM'e nebėra, jokio persidengimo, console klaidų nėra.
- **Vėliava padidinta iki naujo maksimumo be automobilio (2026-07-31)**: kai automobilis pašalintas, atsirado daug daugiau vietos — vėliava padidinta 500×333 → 672×448 (tas pats 1.5 kraštinių santykis), naudojant tą pačią metodiką kaip anksčiau (riboja ilgiausias nelūžtantis antraštės žodis "automobilis?" = 388.75px). Patikrinta 1280px (teksto stulpelis 420px, 31px marža) ir prie pat 1160px ribos (teksto stulpelis 410px, 22px marža) — jokio persidengimo abiem atvejais, esama riba (1160px) liko tinkama be pakeitimų. Console klaidų nėra.
- **Vėliava šiek tiek sumažinta (2026-07-31)**: vartotojas paprašė truputį sumažinti — 672×448 → 580×387 (tas pats santykis). Patikrinta 1280px — teksto stulpelis 512px, jokio persidengimo, console klaidų nėra.
- **Vėliava patraukta į dešinę (2026-07-31)**: `.hero-inner` max-width 1100px → 1140px — visas hero blokas (tekstas+vėliava) tapo 40px platesnis ir liko centruotas, todėl vėliavos dešinysis kraštas pasislinko ~20px į dešinę. **Žinomas šalutinis efektas**: hero blokas dabar 20px platesnis už žemiau esančią paieškos sekciją (`.search-section`, vis dar 1100px), tad kairysis/dešinysis kraštas nebe tiksliai sutampa tarp sekcijų. Patikrinta 1280px ir 1161px (prie pat mobiliojo režimo ribos) — jokio persidengimo, console klaidų nėra.
...
**Papildomai (tą pačią dieną)**: vartotojas atkreipė dėmesį, kad LAIKO (ne datos) pusė vis dar galėjo rodyti AM/PM — `dateTimePairHtml()`'o laiko laukas buvo paliktas kaip native `<input type="time">`, kuris turi TĄ PAČIĄ lokalės problemą kaip `type="date"`. Pasirodo, ŠI TIKSLIAI PROBLEMA jau buvo išspręsta DU KARTUS anksčiau šiame projekte (`automeistrai-login.html`'o `WORK_HOURS_OPTIONS` registracijos formai, dashboard'o paties `TIME_OPTIONS`/`timeSelectHtml()` "Darbo laikas" puslapiui) — abu naudoja `<select>` su iš anksto suformatuotu "HH:MM" tekstu vietoj native laiko lauko. Pritaikytas TAS PATS sprendimas: `dateTimePairHtml()` dabar generuoja `<select>` su dashboard'e JAU esančiu `TIME_OPTIONS` sąrašu (48 pusvalandžio žingsniai, 00:00–23:30) vietoj `<input type="time">` — `combineDateTime()` liko nepakeistas (`<select>.value` veikia identiškai). Patikrinta gyvai: laiko laukas — `<select>` su 48 parinktimis, "17:00" tarp jų, pasirinkus datą+17:00 ir pateikus, DB `scheduled_time` gavo tiksliai `"2026-09-01T17:00"`. `npm test` → 26/26.
...
## Servisų tarpusavio chat — privatus 1-su-1 pokalbis (2026-08-20)

Vartotojas paprašė naujos funkcijos: registruoti servisai gali susisiekti VIENI SU KITAIS (ne su klientais) privačiu susirašinėjimu — pasitarti, perduoti darbą, susisiekti dėl bendradarbiavimo. Pilna užduoties specifikacija apėmė: atskirą chat ženkliuką dashboard'e, serviso paiešką/pasirinkimą, pokalbio langą (vizualus pagrindas — servisucentras-pagrindinis.html svečio chat widget), privatumą (matoma TIK dviem dalyviams; admin mato TIK egzistavimą, ne turinį — pagal nutylėjimą), ir atskirą pranešimų mechanizmą (garsas/blyksėjimas/vibracija).

**DB struktūra** (`schema.sql`) — SĄMONINGAI ATSKIRA nuo `orders`/`order_messages` (ne pakartotinai panaudota su nauju tipu), kad neįterptų klaidingų duomenų į dešimtis esamų vietų, skaičiuojančių/filtruojančių `orders` pagal `client_id` (statistika, kalendorius, admin pajamos): nauja `service_conversations` (service_a_id VISADA < service_b_id — normalizuota pora, UNIQUE apsaugo nuo dviejų atskirų pokalbio įrašų tai pačiai porai) ir `service_chat_messages` (`ON DELETE CASCADE` nuo pokalbio).

**Backend** (`backend/src/routes/serviceChat.routes.js`, prijungtas `/api/service-chat`): `GET /search?q=` (kitų aktyvių, ne-bot servisų paieška pagal pavadinimą/miestą, be savęs), `GET /conversations` (TIK prisijungusio serviso pokalbiai, su paskutinės žinutės peržiūra), `POST /start` (randa arba sukuria pokalbį su `otherServiceId` — normalizuota pora užtikrina, kad abi pusės gauna TĄ PATĮ pokalbį, nepriklausomai kas pradėjo), `GET/POST /:id/messages` — apsaugoti `requireParticipant` middleware'u, kuris tikrina, kad prisijungęs servisas TIKRAI yra vienas iš dviejų dalyvių (403 kitaip) — tai VIENINTELĖ, bet pakankama privatumo apsauga.

**Admin matomumas** (`GET /admin/service-conversations`) — sąmoningai NESELEKTUOJA `message` lauko, tik dalyvių vardus/miestus, žinučių skaičių ir paskutinės žinutės laiką: admin mato, KAD ir KADA du servisai susirašinėjo (ginčų kontekstui), bet NE PATĮ turinį. **Šis "tik egzistavimas" elgesys yra numatytasis pagal aiškų vartotojo nurodymą** — pilna turinio peržiūra admin'ui NEĮGYVENDINTA; jei vėliau prireiktų (pvz. ginčų sprendimui), tai atskiras, sąmoningas pakeitimas. Admin.html UI šiam endpoint'ui kol kas NEPRIDĖTA (analogiškai anksčiau pridėtiems atsiliepimų endpointams be UI) — jei norėsite matomo admin puslapio, pasakykite.

**Duomenų saugojimo politika** (`utils/retention.js`) — 90 dienų automatinis žinučių valymas dabar apima IR `service_chat_messages`, ne tik `order_messages` (ta pati politika, taikoma vienodai).

**Frontend** (`automeistrai-dashboard.html`) — naujas topbar mygtukas 🤝 "Servisų bendruomenė", VIZUALIAI aiškiai atskirtas nuo 💬 kliento pokalbių: mėlynas ženkliukas/rėmelis (`var(--blue)`, `#3A7BD5`) vietoj kliento chat'o raudono/geltono. Slenkantis šoninis panelis (`.s2s-panel`, ne centrinis modalas kaip kliento Chat skydelis) — stiliaus pagrindas paimtas iš servisucentras-pagrindinis.html svečio chat FAB/panelio (`.chat-panel`/`.msg-c`/`.msg-s`), pritaikytas dashboard'o spalvų paletei. Du vaizdai VIENAME panelyje: sąrašo vaizdas (paieška + esamų pokalbių sąrašas su paskutinės žinutės peržiūra ir neskaitytos žymeliu) ir gijos vaizdas (žinučių burbulai + įvesties juosta), perjungiami be atskiro modalo.

**Pranešimų mechanizmas** — TAS PATS 12s poll'inimo ciklas (`pollS2SConversations()` kviečiama `pollOrders()` pabaigoje, ne atskiras laikmatis) ir TAS PATS 3s garso kartojimo ciklas (`checkAlarm()`), bet VIZUALIAI/GARSIŠKAI atskirti nuo kliento pranešimų: `playS2SSound()` groja DU trumpus 600Hz tonus (vietoj vieno 880Hz kliento tono) — servisas iš ausies atskiria "nauja SERVISO žinutė" nuo "nauja KLIENTO žinutė/užklausa", be reikalo žiūrėti į ekraną; pridėta ir `navigator.vibrate()` (feature-detected, tylus no-op, jei naršyklė nepalaiko). Neskaitytų būsena — GRYNAI naršyklės pusėje (localStorage `sc_svcchat_seen_<id>`), ta pati technika kaip kliento pokalbių `computeUnreadOrders()`.

**Testai** (`backend/test/service-chat.test.js`, 4 nauji): rolės apsauga (klientas negali kviesti service-chat endpoint'ų), pati pora negali rašyti sau/be ID, pilnas dviejų servisų susirašinėjimo srautas SU TREČIU (nedalyvaujančiu) servisu, kuris patvirtintai NEI mato pokalbio sąraše, NEI gali skaityti/rašyti žinutes (403), IR admin egzistavimo-be-turinio patikra (žinutės tekstas TIKRINAMAS, kad neatsirastų JOKIA forma admin atsakyme). Pakeliui rasta ir ištaisyta lenktynių sąlyga: `created_at` turi tik sekundės tikslumą, tad kelios žinutės tą pačią sekundę susimaišydavo `ORDER BY created_at DESC LIMIT 1` rikiavime — pridėtas `id` kaip antrinis raktas (ta pati klasė bug egzistuoja ir keliose SENESNĖSE `orders.routes.js`/`admin.routes.js` vietose, bet jos ŠIO task'o apimtyje NELIESTOS).

**Patikrinta gyvai**: sukurti 3 testiniai servisai (A/B/C) tame pačiame mieste. A per paiešką susirado B, atidarė/sukūrė pokalbį, parašė žinutę — B (atskirame naršyklės tab'e) po poll'inimo pamatė ženkliuką+mirksėjimą, atsidarius pokalbį matė A žinutę, atsakė. A poll'inimu pamatė B atsakymą gijoje. C prisijungus TREČIU servisu — pokalbių sąrašas tuščias ("Pokalbių dar nėra"), o tiesioginis bandymas per API pasiekti `/service-chat/1/messages` grąžino 403 "Šis pokalbis jums nepriklauso". Ženkliukų spalvos patvirtintos skirtingos (`rgb(58,123,213)` mėlyna S2S vs `rgb(139,26,26)` tamsiai raudona kliento). Pakeliui rastas ir ištaisytas realaus naudojimo bug'as — po žinutės išsiuntimo fokusas lieka ant "Siųsti" mygtuko, kuris IRGI yra gijos konteinerio viduje, tad `typingInPanel` apsauga klaidingai sustabdydavo gijos atnaujinimą; pataisyta tikrinant TIK patį teksto lauką (`#s2s-input`), ne visą konteinerį. Visi testiniai įrašai išvalyti po patikrinimo. `npm test` → 30/30 (buvo 26).
...
**Apimtis — UŽBAIGTA VISOMS 30 MARKIŲ** (pratęsta po pirminio 5-markių etapo, vartotojui patvirtinus "taip" tęsti): **visos** `CAR_MAKES_MODELS` markės dabar turi kartos duomenis — Volkswagen, Audi, BMW, Toyota, Škoda, Chevrolet, Citroën, Dacia, Fiat, Ford, Honda, Hyundai, Jeep, Kia, Land Rover, Lexus, Mazda, Mercedes-Benz, Mini, Mitsubishi, Nissan, Opel, Peugeot, Porsche, Renault, Seat, Subaru, Suzuki, Volvo, Lada — iš viso **165 kartos įrašų** 30 markių. Modeliams be tikslių papildomų duomenų (retesni/nišiniai variantai kai kuriose markėse) elgesys lieka senas — laisvai bet kurie metai.

**Rasta ir ištaisyta duomenų klaida savikontrolės metu**: parašiau automatinį validatorių (Node `vm` modulis, tikrina kiekvienos markės/modelio kartų sąrašą dėl PERSIDENGIANČIŲ metų intervalų — pvz. jei dvi kartos abi "apima" tuos pačius metus, `findMatchingGeneration()` visada rastų PIRMĄ (senesnę), o ne teisingą). Rasta **12 realių persidengimų** (BMW 3/5/6 serija, Z4, Toyota Corolla, Opel Astra, Subaru Forester ir kt.) — visi ištaisyti koreguojant ribines metų reikšmes. Likę 3 informaciniai "tarpai" (BMW Z4 2016–2018, Hyundai Tucson 2010–2015, Volvo V40 2004–2012) yra TIKRI, istoriškai tikslūs gamybos pertrūkiai (modelis laikinai nebuvo gaminamas arba pervadintas), palikti tyčia, ne klaida.
...
**Patikrinta gyvai**: VW Golf pasirinkus "Golf · Mk3 (1991–1997)" — Metų sąrašas iškart susiaurėjo į 1991–1997; `readCarSelection()` grąžino švarų `{make:'Volkswagen', model:'Golf', year:1995}`. Markei BE kartos duomenų — modelio sąrašas liko nepakitęs (paprasti pavadinimai). Redaguojant jau įrašytą automobilį (BMW "3 serija", 2007) — sistema automatiškai pažymėjo TEISINGĄ kartą "E90/E91/E92/E93 (2005–2011, po pataisymo)" ir susiaurino metų sąrašą, be jokio papildomo vartotojo veiksmo. Po pilno išplėtimo — patikrinti IR Mercedes-Benz "C klasė" (5 kartos W202→W206), IR Lada "Niva" (2 kartos), visos rodomos teisingai. `car-data.js` sintaksė patikrinta (`node --check`), automatinis persidengimo/tarpo validatorius parašytas ir paleistas (0 persidengimų liko). `npm test` → 37/37 (backend nekeistas — šis pakeitimas grynai frontend).
